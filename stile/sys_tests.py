...
class CorrelationFunctionSysTest(SysTest):
    ...
    plot_types = [PlotDetails(t_field='omega', t_title='$\omega$', 
    ...
    def plot(self, data, colors=['r', 'b'], log_yscale=False,
        ...
        ax.errorbar(data[r], data[pd.t_field], yerr=data[pd.sigma], color=colors[0], 
                    label=pd.t_title)
        if pd.b_title and plot_bmode:
            ax.errorbar(data[r], data[pd.b_field], yerr=data[pd.sigma], color=colors[1], 
                        label=pd.b_title)
        elif pf.t_im_title:  # Plot y and y_im if not plotting yb (else it goes on a separate plot)
            ax.errorbar(data[r], data[pd.t_im_field], yerr=data[pd.sigma], color=colors[1], 
                        label=pd.t_im_title)
        ax.set_xscale('log')
        ...
        if pd.b_field and plot_bmode and pd.t_im_field:  
            ...
            ax.errorbar(data[r], data[pd.t_im_field], yerr=data[pd.sigma], color=colors[0], 
                        label=pd.t_im_title)
            ax.errorbar(data[r], data[pd.b_im_field], yerr=data[pd.sigma], color=colors[1], 
                        label=pd.b_im_title)
            ax.set_xscale('log')
            ax.set_yscale(yscale)
            ax.set_xlim(xlim)
        ...
        if plot_data_only and pd.datarandom_t_field:  # Plot the data-only measurements if requested
            curr_plot += 1
            ax = fig.add_subplot(nrows, 1, 2)
            ax.errorbar(data[r], data[pd.datarandom_t_field+'d'], yerr=data[pd.sigma], 
                        color=colors[0], label=pd.datarandom_t_title+'d}$')
            if plot_bmode and pd.datarandom_b_field:
                ax.errorbar(data[r], data[pd.datarandom_b_field+'d'], yerr=data[pd.sigma], 
        ...
        if plot_random_only and dr_y:  # Plot the randoms-only measurements if requested
            ...
            ax.errorbar(data[r], data[pd.datarandom_t_field+'r'], yerr=data[pd.sigma], 
                        color=colors[0], label=pd.datarandom_t_title+'r}$')
            if plot_bmode and pd.datarandom_b_field:
                ax.errorbar(data[r], data[pd.datarandom_b_field+'r'], yerr=data[pd.sigma], 
