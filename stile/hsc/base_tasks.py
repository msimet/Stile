...
class CCDSingleEpochStileTask(lsst.pipe.base.CmdLineTask):
    ...
    def generateColumns(self, dataRef, catalog, mask, raw_cols, extra_col_dict):
        ...
        calib_type, calib_data, calib_metadata = self.getCalibData(dataRef, shape_cols)
    ...
    def getCalibData(self, dataRef, shape_cols):
        ...
        try:
            if dataRef.datasetExists("fcr_md"):
                if shape_cols:
                    calib_data = dataRef.get("calexp", immediate=True)  # only used for WCS
                calib_metadata = dataRef.get("fcr_md", immediate=True)
                calib_type = "fcr"
            else:
                if shape_cols:
                    calib_data = dataRef.get("calexp", immediate=True)
