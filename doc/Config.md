In the rest of these examples, we assume that you have installed PyYAML, which allows the reading of YAML configuration files.  Everything that follows will also work with the builtin JSON, once the format has been correctly set up.
...
Stile.py is found in the topmost directory of the Stile package you downloaded from Github.
...
We expect a list of files in a configuration file with appropriate descriptions of what's in them.  You can specify this multiple times to describe different data sets, with the following caveats:
    - You can't use the same key in the config file more than once, so you should number your file descriptions: "file0", "file1", etc.  Just make sure the first four characters are "file" and Stile will interpret it for you.
 ...
 'catalog':
   'single':
     ...
     'field':
       'star': star3.fits
       'galaxy': galaxy3.fits
Note that if you replace one of the filename strings with a dict, you should put the filename string in the key "name".
